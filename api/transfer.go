...
import (
	...
	db "github.com/morticius/golang_bank/db/sqlc"
)

type transferRequest struct {
	FromAccountID int64  `json:"from_account_id" binding:"required,min=1"`
	ToAccountID   int64  `json:"to_account_id" binding:"required,min=1"`
	Amout         int64  `json:"amount" binding:"required,gt=0"`
...
func (server *Server) validAccount(c *gin.Context, accountID int64, currency string) bool {
	...
	if account.Currency != currency {
		err := fmt.Errorf("account [%d] currency mismaych: %s vs %s", account.ID, account.Currency, currency)
		c.JSON(http.StatusBadRequest, errorResponse(err))
		return false
	}
